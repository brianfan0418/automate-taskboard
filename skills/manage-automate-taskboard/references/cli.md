...
Set `CODEX_TASKBOARD_URL` to override the default local API origin, `http://127.0.0.1:47833`.
...
Without `--import`, issues created through `taskctl` always start in `backlog`. `--import` is only for importing existing project task status and only works from this device: `--status done` creates a done issue, every other status creates a `backlog` issue, and imported issues never start or queue AI work.
