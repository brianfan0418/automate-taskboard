...
export function taskStatusFromJira(status) {
  ...
  if (category === "done") {
    return includesAny(name, ["cancel", "reject", "取消", "拒绝", "拒絕"]) ? "canceled" : "done";
  }
  if (category === "new") {
    return includesAny(name, ["backlog", "待立项", "待立項", "需求池"]) ? "backlog" : "todo";
  }
  if (includesAny(name, ["review", "verify", "test", "验收", "驗收", "评审", "評審", "测试", "測試"])) {
    return "in_review";
  }
  if (includesAny(name, ["block", "hold", "阻塞", "挂起", "掛起"])) return "blocked";
  return "in_progress";
}

export function taskPriorityFromJira(priority) {
  const name = priority?.name ?? "";
  if (includesAny(name, ["highest", "critical", "blocker", "urgent", "紧急", "緊急", "最高"])) {
    return "urgent";
  }
...
function jiraOriginId(manifest) {
  ...
  if (!applicationId) {
    throw new ApiError(502, "INVALID_JIRA_ORIGIN", "Jira 未傳回穩定的實例身分");
  }
  ...
  async function request(config, pathname, init = {}) {
    ...
    try {
      ...
      throw new ApiError(
        ...
        timedOut ? "連接 Jira 超時" : "無法連接 Jira，請檢查地址和內部網路連線",
      );
    } finally {
    ...
    if (response.status >= 300 && response.status < 400) {
      throw new ApiError(400, "JIRA_REDIRECT", "Jira 地址發生重新導向，請填寫最終存取地址");
    }
    if (!response.ok) {
      throw new ApiError(
    ...
    try {
      ...
      throw new ApiError(502, "INVALID_JIRA_RESPONSE", "Jira 傳回了無效的 JSON 資料");
    }
  }
