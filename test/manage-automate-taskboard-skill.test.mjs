...
const skillSource = await readFile(
  new URL("../skills/manage-automate-taskboard/SKILL.md", import.meta.url),
  "utf8",
);
const cliReference = await readFile(
  new URL("../skills/manage-automate-taskboard/references/cli.md", import.meta.url),
  "utf8",
);
...
test("the taskboard skill imports task status only through the loopback import flag", () => {
  assert.match(skillSource, /## Import existing project task status/);
  assert.match(skillSource, /taskctl issue create --import --project PROJECT_ID/);
  assert.match(skillSource, /`--status done` only for work the conversations show as completed/);
  assert.match(skillSource, /never move imported issues to `todo` or `in_progress`/);
  assert.match(cliReference, /\[--import\]/);
  assert.match(cliReference, /Without `--import`, issues created through `taskctl` always start in `backlog`/);
});
